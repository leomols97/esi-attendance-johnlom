...
@section('content')
    ...
    <h1>Les étudiants</h1>
    <p><label dusk="seance_title">Cours: {{$seance[0]->title}}</label></p>
    <p><label dusk="seance_teacher">Professeur: {{$seance[0]->teacher}}</label></p>
    <p><label dusk="seance_date">Date et heure: {{$seance[0]->start}}</label></p>
            ...
            @foreach ($students as $student)
                <tr>
                    ...
                    @if(isset($presences[$student->id]) && $presences[$student->id])
                        <td><input type="checkbox" name="checklist[{{$student->id}}]" id="{{$student->id}}"
                                   dusk="{{$student->id}}" checked></td>
                    @else
                        <td><input type="checkbox" name="checklist[{{$student->id}}]" id="{{$student->id}}"
                                   dusk="{{$student->id}}"></td>
                    @endif
